...
class IsingIntegrator(object):
    ...
    def integrate(self,
        ...
        E0 = e_J+e_B
        if verbose: print("Step= %4d Energy= %+1.4e [e_B=%+1.4e e_J=%+1.4e]" % (
            0, E0, e_B, e_J))
        # Annealing
        if anneal.shape[0] == 0:
        ...
        for n in range(n_steps):
            ...
            e_B, e_J = model.energy_split(state)
            E = e_J+e_B
            if verbose: print("Step= %4d Energy= %+1.4e [e_B=%+1.4e e_J=%+1.4e]   T=%+1.3e" % (
                n, E, e_B, e_J, kT))
...
if __name__ == "__main__":
    ...
    state = model.spawn()
    integrator = IsingIntegrator({"kT": 20.})
    n_steps = 10100
    sample_every = 100
    data = integrator.integrate(model, state, n_steps, 
        sample_every=sample_every, 
        anneal=np.linspace(100.,0.1,n_steps))
    X = np.array([ d.S for d in data.states ])
    y = np.array([ d.E for d in data.states ])
    bml.write(
        "data.extt", 
        arrays={"X": X, "Y": y, "J": model.J, "V": model.J_modes},
        meta=model.config)

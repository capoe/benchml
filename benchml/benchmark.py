...
def evaluate_model(dataset, model, accu=None, log=log, verbose=False, detailed=True):
    if accu is None:
    ...
    for split_args in dataset["splits"]:
        ...
        for stream_train, stream_test in stream.split(**split_args):
            train_id = make_split_id(
                dataset["splits"][0]["method"], "train", stream_train, stream_test
            )
            test_id = make_split_id(
                dataset["splits"][0]["method"], "test", stream_train, stream_test
            )
    ...
    t_out = time.time()
    perf = accu.evaluateAll(metrics=dataset.meta["metrics"], bootstrap=100, log=log)
...
def evaluate_ensemble(dataset, models, log, verbose=False, detailed=False):
    ...
    for model in models:
        ...
        record = evaluate_model(dataset, model, log=log, verbose=verbose, detailed=detailed)

...
def analyse_section(split_this, benchmark_section, return_ordered=False):
    ...
    for record in benchmark_section:
        splits = filter(lambda s: s["id"] == split_this["id"], read_split_props(record["splits"]))
        for split in splits:
            ...
            s = [(+1 if Accumulator.select_best[key] == "smallest" else -1) for key in metrics]
            P.append(p)
            P_std.append(p_std)
            S.append(s)
    P = np.array(P)
...
def analyse(benchmark):
    ...
    for section_name, benchmark_section in sorted(sections.items()):
        ...
        splits_section = sorted(list(set([s for r in benchmark_section for s in r["splits"]])))

...
class AsapXyz(AsapTransform):
    ...
    def _fit(self, inputs):
        asapxyz = asaplib.data.ASAPXYZ(
            ...
            periodic=inputs["meta"]["periodic"] if "periodic" in inputs["meta"] else False,
        )
        soap_js = {
            "soap1": {
                "type": "SOAP",
        ...
        kernel_js = {}
        kernel_js["k1"] = {"kernel_type": "moment_average", "zeta": 2, "element_wise": False}
        kernel_js["k2"] = {"kernel_type": "sum", "element_wise": True}
        ...
        asapxyz.compute_global_descriptors(
            desc_spec_js, sbs=[], keep_atomic=inputs["meta"]["per_atom"]
        )

...
class PhyschemXtal(Transform):
    default_args = {"bins": 10, "sigma_fac": 1.0}
    allow_stream = ("X",)
    stream_samples = ("X",)
    req_inputs = ("configs",)
    precompute = True
    fct_type = {"discrete": "heaviside", "continuous": "gaussian"}
...
class Physchem2D(Transform):
    ...
    def _setup(self, *args, **kwargs):
        if self.args["select"] is not None:
            ...
            select = set(
                self.predefined[self.args["select_predef"]]
            )  # Invalid arg in Physchem2D.select_set?
        self.descriptors_active = list(filter(lambda d: d[0] in select, self.descriptors))

...
def parse_single(section, verbose=True):
    ...
    model = section["model"]
    if verbose: print("Parse section: dataset=%s, model=%s" % (dataset,model))
    ...
    for split in splits_test:        
        if verbose: print("  - We are now parsing the predictions for the split with train_fraction=%1.2f" % split["train_fraction"])
        n_repeats = len(section["output"][split["id"]])
        if verbose: print(" (repeated %d times)" % n_repeats)
        output_by_train_test_ratio[split["train:test"]] = {"train_fraction": split["train_fraction"],
                                                           "n_repeats": n_repeats}
        #print(" Collate test indices, test predictions, test targets")
        test_idcs = np.array(list(itertools.chain(*[ slice for slice in section["slices"][split["id"]]]))).reshape((-1,1))
        y_test_pred = np.array(list(itertools.chain(*[ out["pred"] for out in section["output"][split["id"]] ]))).reshape((-1,1))
        y_test_true = np.array(list(itertools.chain(*[ out["true"] for out in section["output"][split["id"]] ]))).reshape((-1,1))
        #print(np.shape(test_idcs))
        collated = np.concatenate([ test_idcs, y_test_pred, y_test_true, 
                                   np.repeat(np.arange(n_repeats), len(test_idcs)/n_repeats).reshape((-1,1)),
                                  np.zeros(len(test_idcs)).reshape((-1,1))], axis=1)
    ...
    for split in splits_train:
        ...
        train_idcs = np.array(list(itertools.chain(*[ slice for slice in section["slices"][split["id"]]]))).reshape((-1,1))
        y_train_pred = np.array(list(itertools.chain(*[ out["pred"] for out in section["output"][split["id"]] ]))).reshape((-1,1))
        y_train_true = np.array(list(itertools.chain(*[ out["true"] for out in section["output"][split["id"]] ]))).reshape((-1,1))
        collated = np.concatenate([ train_idcs, y_train_pred, y_train_true, 
                                   np.repeat(np.arange(n_repeats), len(train_idcs)/n_repeats).reshape((-1,1)),
                                  np.ones(len(train_idcs)).reshape((-1,1))], axis=1)

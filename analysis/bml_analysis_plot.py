...
def plot_compare_train_pred(
    ...
    fig, ax = plt.subplots(
        ncols=2,
        nrows=len(by_model[model_key_now].keys()),
    ...
    for i_now, key_now in enumerate(by_model[model_key_now].keys()):
        ...
        n, bins, _ = ax[i_now, 1].hist(
            pred_error, histogram_bins, density=True, alpha=0.75, histtype="step", label="train"
        )
        pred_error_min, pred_error_max = min(np.amin(bins), pred_error_min), max(
            np.amax(bins), pred_error_max
        )
        ...
        n, bins, _ = ax[i_now, 1].hist(
            pred_error, histogram_bins, density=True, alpha=0.75, histtype="step", label="test"
        )
    ...
    if save:
        fig.savefig(file_prefix + model_key_now + ".pdf")
...
def get_learning_curve(
    ...
    lc_train_scores = LC_SCOREBOARD(
        [len(v[1]["train"]) / v[1]["n_repeats"] for v in by_model[model_key_now].items()]
    )
    ...
    for i_now, key_now in enumerate(by_model[model_key_now].keys()):
        ...
        for n_now in range(n_repeats):
            ...
            lc_score_now = get_score(
                np.asarray(sub_model_train[:, 0]), np.asarray(sub_model_train[:, 1])
            )
            lc_train_scores.add_score(n_train, lc_score_now)

            lc_score_now = get_score(
                np.asarray(sub_model_test[:, 0]), np.asarray(sub_model_test[:, 1])
            )
